...
* [GeodeBackgroundBRepEpsilonOptimizerException](GeodeBackgroundBRepEpsilonOptimizerException.md)
* [GeodeBackgroundSolidEpsilonOptimizerException](GeodeBackgroundSolidEpsilonOptimizerException.md)
* [GeodeBackgroundSurfaceEpsilonOptimizerException](GeodeBackgroundSurfaceEpsilonOptimizerException.md)
...
```cpp
BackgroundSurfaceModifier::CleanMappings carve_background_surface(ModifiableBackgroundSurface background_surface)
```
...
```cpp
std::vector<geode::index_t> simplify_background_solid_multilayers(BackgroundSolidConstraintModifier & constraint_modifier, TetrahedralSolidBuilder3D & builder, absl::Span<const index_t> tetrahedra, const Plane & plane)
```
...
```cpp
void epsilon_optimize_background_surface_elements(BackgroundSurfaceConstraintModifier & constraint_modifier, absl::Span<const index_t> triangles)
```
...
```cpp
std::optional<std::tuple<MultilayerTetrahedra, OwnerPlane>> detect_coplanar_multilayers(const TetrahedralSolid3D & solid, index_t tetrahedron_id)
```
...
```cpp
std::vector<index_t> epsilon_optimize_background_solid_elements(BackgroundSolidConstraintModifier & constraint_modifier, TetrahedralSolidBuilder3D & builder, absl::Span<const index_t> tetrahedra)
```
...
```cpp
void shift_optimize_background_solid_elements(BackgroundSolidConstraintModifier & constraint_modifier, const TetrahedralSolidInternalDistanceImprovementSimulator & improvement_simulator, absl::Span<const index_t> elements)
```
...
```cpp
void optimize_background_solid_elements(BackgroundSolidConstraintModifier & constraint_modifier, const TetrahedralSolidInternalDistanceImprovementSimulator & improvement_simulator, absl::Span<const index_t> elements)
```
...
```cpp
std::vector<BlockElement<index_t>> optimize_background_brep_elements(BackgroundBRepGeometricConstraintModifier & constraint_modifier, const BRepInternalDistanceImprovementSimulator & improvement_simulator, absl::Span<const BlockElement<index_t>> tetrahedra)
```
...
```cpp
void blinded_quality_optimize_background_solid_elements(BackgroundSolidConstraintModifier & constraint_modifier, const TetrahedralSolidInternalDistanceImprovementSimulator & improvement_simulator, absl::Span<const index_t> elements)
```
...
```cpp
SurfaceSplitsOrders find_split_orders(CorafinatedSurface & corafinated_surface, SurfaceSkins & skins)
```
...
```cpp
BijectiveMapping<MeshVertex> vertex_skins_solid_mapping(const CorafinatedSurface & corafinated_surface, geode::internal::BackgroundSolidBuilder & solid_builder, const SurfaceSkins & skins)
```
...
```cpp
BijectiveMapping<MeshVertex> vertex_skins_solid_mapping(const CorafinatedSurface & corafinated_surface, const SurfaceSkins & skins)
```
