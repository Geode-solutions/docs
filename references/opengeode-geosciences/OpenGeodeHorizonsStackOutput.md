...
```cpp
public void OpenGeodeHorizonsStackOutput<dimension>(std::string_view filename)
```
...
```cpp
public void save_horizons_stack_files(const HorizonsStack<dimension> & horizons_stack, std::string_view directory)
```
...
```cpp
public std::vector<std::string> write(const HorizonsStack<dimension> & horizons_stack)
```
